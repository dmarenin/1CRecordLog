...
def res_to_vis_js(data, date, cache_content, dep_work_time, list_time_reserved, r_ref, is_garant, empl_dev):
    ...
    eq = ['11','12','13', '14', '15', '16', '17']

    className = 'empty'

    style = "color: white; background-color: #999999;"

    content = 'Блок работ'+'<BR>'+''

    d = {'content':content, 'id':str(uuid.uuid1()),  'className': className, 'style': style, 'nestedGroups':eq, 'showNested': False, 'order':100}

    timeline['groups'].append(d)


    timeline['groups'].append({'id':'11', 'content': 'Диагностика  - (пост 2)', 'order':104, 'style' : style})
    ...
    while date_start<date_stop:
        d5 = {'start' : date_start.strftime("%Y-%m-%dT%H:%M:00.000"), 
             'end' :  (date_start+timedelta(minutes=60)).strftime("%Y-%m-%dT%H:%M:00.000"), 
             'group' : 11, 
             'content' : '<BR>',                
             'style' : style1,                
             'title': '<BR>',
        ...
        date_start = date_start+timedelta(minutes=60)


    #timedelta(minutes=time_acceptions)

    #for x in 



    timeline['groups'].append({'id':'12', 'content': 'Шиномонтаж - (пост 1)', 'order':101, 'style' : style,})
    ...
    while date_start<date_stop:
        d5 = {'start' : date_start.strftime("%Y-%m-%dT%H:%M:00.000"), 
             'end' :  (date_start+timedelta(minutes=30)).strftime("%Y-%m-%dT%H:%M:00.000"), 
             'group' : 12, 
             'content' : '<BR>',                
             'title': '<BR>',
             'style' : style1,   
    ...
    timeline['groups'].append({'id':'13', 'content': 'Шиномонтаж - (пост 2)', 'order':102, 'style' : style,})
    ...
    timeline['groups'].append({'id':'14', 'content': 'Техническое обслуживание - (пост 1)', 'order':105, 'style' : style,})
    ...
    while date_start<date_stop:
        d5 = {'start' : date_start.strftime("%Y-%m-%dT%H:%M:00.000"), 
             'end' :  (date_start+timedelta(minutes=90)).strftime("%Y-%m-%dT%H:%M:00.000"), 
             'group' : '14', 
             'content' : '<BR>',                
             'title': '<BR>',
             'className' : class_name,
        ...
        date_start = date_start+timedelta(minutes=90)
          
    timeline['groups'].append({'id':'15', 'content': 'Техническое обслуживание - (пост 2)', 'order':106, 'style' : style,})
    ...
    while date_start<date_stop:
        d5 = {'start' : date_start.strftime("%Y-%m-%dT%H:%M:00.000"), 
             'end' :  (date_start+timedelta(minutes=270)).strftime("%Y-%m-%dT%H:%M:00.000"), 
             'group' : '15', 
             'content' : '<BR>',                
             'title': '<BR>',
             'className' : class_name,
        ...
        date_start = date_start+timedelta(minutes=270)
    
    timeline['groups'].append({'id':'16', 'content': 'Диагностика - (пост 1)', 'order':103, 'style' : style,})
    ...
    while date_start<date_stop:
        d5 = {'start' : date_start.strftime("%Y-%m-%dT%H:%M:00.000"), 
             'end' :  (date_start+timedelta(minutes=180)).strftime("%Y-%m-%dT%H:%M:00.000"), 
             'group' : '16', 
             'content' : '<BR>',                
             'title': '<BR>',
             'style' : style1,  
        ...
        date_start = date_start+timedelta(minutes=180)
    
    date_start = date
    date_stop = date+timedelta(days=1)

    timeline['groups'].append({'id':'17', 'content': 'Посты', 'order':110, 'style' : style,})

    while date_start<date_stop:
        d5 = {'start' : date_start.strftime("%Y-%m-%dT%H:%M:00.000"), 
             'end' :  (date_start+timedelta(minutes=360)).strftime("%Y-%m-%dT%H:%M:00.000"), 
             'group' : '17', 
             'content' : '<BR>',                
             'title': '<BR>',
             'className' : class_name,
        ...
        date_start = date_start+timedelta(minutes=360)
...
def get_mssql_conn():
    ...
    conn = pymssql.connect(server='', user='sa', password='*', database='ex_working_hours')  
    cursor = conn.cursor()  
